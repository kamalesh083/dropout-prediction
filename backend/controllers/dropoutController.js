...
const predictDropout = async (req, res) => {
  try {
    ...
    const features = {
      test_completion_rate: studentData.test_completion_rate,
      notifications_ignored: studentData.notifications_ignored,
      course_progress: studentData.course_progress,
    };

    // Validate that the features are not missing
    if (
      !features.test_completion_rate ||
      !features.notifications_ignored ||
      !features.course_progress
    ) {
    ...
    pythonProcess.stdout.on("data", (data) => {
      if (!isResponseSent) {
        ...
        res.json({
          ...
          prediction: prediction, // The prediction made by Python
        });
      }
    });
    ...
    pythonProcess.stderr.on("data", (data) => {
      if (!isResponseSent) {
        ...
        res
          .status(500)
          .json({ message: `Error in prediction: ${data.toString()}` });
      }
    });
    ...
    pythonProcess.on("close", (code) => {
      if (!isResponseSent) {
        ...
        if (code !== 0) {
          res
            .status(500)
            .json({ message: "Python script failed with code " + code });
        }
      }
    });
  } catch (error) {
